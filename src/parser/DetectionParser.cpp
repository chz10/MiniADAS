...
ParseResult DetectionParser::parseFile(
    ...
    if (!input.is_open()) {
        ...
        result.message = "Failed to open Detection file: " + file_path;
        return result;
    }
    ...
    while (std::getline(input, line)) {
        ...
        if (!parseDataLine(trimmed_line, confidence_threshold, object)) {
            ++skipped_lines;
            continue;
        }
    ...
    if (line_count == 0 || parsed_objects == 0) {
        ParseResult result;
        result.success = false;
        result.message = "No valid Detection objects parsed from file: " + file_path;
    ...
    ParseResult result;
    result.success = true;
    result.message = "Parsed Detection file successfully: " + file_path;
    result.parsed_objects = parsed_objects;
    result.skipped_lines = skipped_lines;
...
bool DetectionParser::parseDataLine(
    ...
    try {
        ...
        object.confidence = std::stod(fields[3]);
        object.bbox.x1 = std::stod(fields[4]);
        object.bbox.y1 = std::stod(fields[5]);
        object.bbox.x2 = std::stod(fields[6]);
        object.bbox.y2 = std::stod(fields[7]);
        object.distance = std::stod(fields[8]);
    } catch (...) {
        return false;
    }
